...
export default function Form({ canSubmit, userEntry, appState, fetchCats, categories, handleChange, handleSubmit, setUserEntry, setGetCats }: FormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className='monstera px-10 md:py-8 flex flex-col items-center gap-2 md:rounded-md md:shadow-2xl shadow-slate-950'>
    <h1 className='text-[#de79f7] hidden md:block text-xl py-4 font-bold self-start'>Register</h1>
      ...
      <div className='flex flex-col gap-4 text-sm w-full py-4'>
        ...
        <div className='relative flex items-center gap-2 w-full'>
          ...
          <div className='flex-auto flex flex-col w-full gap-1'>
            <label htmlFor='cat' className='text-[13px]'>Category</label>
            <select
              id='cat'
              className='flex-auto bg-inherit cursor-pointer placeholder:text-gray-600 rounded-[4px] p-2 focus:outline-none w-full px-2 border border-gray-300'
              onClick={() => setGetCats('YES')}
              onChange={event => setUserEntry(prev => ({ ...prev, category: +event.target.value }))}
            >
              <option value="" className='text-white text-center bg-[#150E28]'>Select your category</option>
              {
                categories?.map(cat => (
                  <option 
          ...
          <div className='md:flex-auto flex-none flex flex-col w-1/4 md:w-full gap-1'>
            <label htmlFor='group' className='text-[13px]'>Group Size</label>
            <select
              id='group'
              className='flex-none bg-inherit cursor-pointer placeholder:text-gray-600 rounded-[4px] p-2 focus:outline-none w-full px-2 border border-gray-300'
              onChange={event => setUserEntry(prev => ({ ...prev, group_size: +event.target.value }))}
            >
              <option value="" className='text-white text-center bg-[#150E28]'>Select</option>
              {
      ...
      <div className='flex gap-3'>
        <input 
          ...
          className='w-4 h-4 focus:outline-none bg-[#333333]'
        />
        <span className='text-[12px]'>I agreed with the event terms and conditions and privacy policy</span>
      </div>

      <button 
        type='submit'
        disabled={!canSubmit && !isLoading}
        className={`${(!canSubmit || isLoading ) ? 'bg-gradient-to-r to-indigo-500 from-gray-500' : 'bg-gradient-to-r to-indigo-500 from-pink-500'} rounded-[5px] text-xs self-center p-3.5 px-16 w-fit md:w-full hover:opacity-90 transition-all`}>
...
const FieldInput = ({ type='text', value, title, placeholder, name, handleChange }: FieldInputProp) => {

  return (
    <div className='flex flex-col w-full gap-1'>
      <label htmlFor={name} className='text-[13px]'>{title}</label>
      <input 
        type={type} 
        id={name}
