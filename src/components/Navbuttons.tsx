...
export default function Navbuttons({ screen }: NavbuttonsProps) {
  ...
  return (
    <div className={`${screen === 'MAX' ? 'hidden md:flex md:flex-row items-center' : 'flex flex-col pr-10 items-start'} gap-8`}>
      ...
      <Link to={'/register'} className={`${pathname === '/register' ? 'border border-t-indigo-600 border-b-pink-500 border-l-[#760865] border-r-[#3a1153]' : 'bg-gradient-to-r to-indigo-500 from-pink-500'} rounded-[4px] text-xs p-3 px-14 md:px-7 md:p-2.5 w-fit hover:opacity-90 transition-all`}>
        Register
      </Link>
    </div>
  )
